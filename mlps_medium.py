...
class MediumNeuralNetwork(nn.Module):
    def __init__(self, img_size, dropout_rate, in_channels_thickness=1): # W * H * T
        ...
        self.linear_relu_stack = nn.Sequential(
            nn.Linear(in_channels_thickness * img_size * img_size, 512),
            nn.ReLU(),
...
# Medium
def medium_nn_training(dataset_name):
    max_acc = float('-inf')
    winner = {}
        ...
        for optimizer_name in ["sgd","adam"]:
            for lr in [0.01, 0.001, 0.0001]:
                for dropout_rate in [0.2, 0.5]:
                    ...
                    # nn medium model
                    model = MediumNeuralNetwork(pixel_size, dropout_rate, in_channels_thickness).to(device)
    ...
    # final training
    model_final = MediumNeuralNetwork(pixel_size, winner["dropout_rate"], in_channels_thickness).to(device)
